 ...
 * persistent chat-context segment of the status strip (spec 1.5), the
...
export interface ExecutorInternals {
  ...
  /** The conversation the terminal is focused on (spec 1.5). null = the home
  ...
  /** Trust-on-first-use mode (spec 4.6a, /settings trust). When true
  ...
  connectWs(): void;
  /** Restart the idle auto-lock countdown (spec 2.4). */
  touchAutoLock(): void;
}
 ...
 * use is on (spec 4.6a). Returns the updated Contact; the caller persists it. */
