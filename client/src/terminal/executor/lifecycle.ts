// Message lifecycle (spec 5.1-5.3a): the mutual disappearing timer, the
...
// not forensic erasure (spec 5.4, 7.12).
 ...
 * retention cap, whichever bites first (spec 5.2-5.3). Best-effort, on
 * activity. */
...
/** `/purge set <duration|off>`: local retention cap, never transmitted (spec 5.3). */
...
/** `/purge now [alias]`: immediate local deletion of stored messages (spec 5.3). */
 ...
 * BOTH sides (spec 5.3a). `last`/`N`/`all` scope to the active conversation;
...
/** A peer asked us to delete messages they had sent us (spec 5.3a). Remove
